...
public class Filter1 implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            //cho nay de xu ly request truoc khi xuong tang tiep theo
            int data = Integer.parseInt(request.getParameter("txtnum"));
            if(data > 0)
                chain.doFilter(request, response);
            else
                ((HttpServletResponse) response).sendRedirect("page1.jsp");
            chain.doFilter(request, response);
            //cho nay xu ly response truoc khi gui qua tang tren
